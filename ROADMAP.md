...
### Тема: Progress (Таб 4 - Статистика) [PRG]
- [x] `[P3.PRG.1]` Система Достижений (Achievements/Badges).
...
- [x] `[P3.PRG.1]` Проектирование Моделей Данных (+ public/private статус). *(Модели TrainingProgram и ProgramStep созданы)*.
- [ ] `[P3.PRG.2]` Реализация UI Создания/Редактирования Программ.
- [~] `[P3.PRG.3]` Реализация `ProgramService` (сохранение/загрузка). *(Протокол и заглушка класса созданы)*.
...
### Тема: RPG Механики [RPG]
- [x] `[P3.RPG.1]` Реализация Бонуса к XP от Аттрибутов: Логика расчета в `ProgressService` + применение в `Leveling` `[P1.LVL.5]`. Зависит от `[P2.RPG.1]`. *(Реализовано)*.
...
- [~] `[P3.MSG.1]` Бэкенд: Схема БД, Real-time, API. *(Модели Chat и ChatMessage, протокол и заглушка MessagingService созданы. Логика Firestore отсутствует)*.
